...
- After completing a significant task or at session end, append **one concise project-state entry** to the resolved `.globalcontext.md` using the prefix `Gemini:` and the current UTC date/time, then run `globalcontext checkpoint-complete --clear-only` to remove the pending marker.
