---
name: globalcontext
description: Auto-load, create, and update .globalcontext.md to share context across terminal AI assistants (Claude, Kimi, Codex, Gemini).
---

# Global Context

Global Context shares project context across terminal AI assistants (Claude Code, Kimi Code CLI, OpenAI Codex CLI, Gemini CLI, etc.).
...
3. If the user gives a project path directly, run:
   ...
   ```
4. If the user proposes a **new project** without specifying a path (e.g. "vamos a hacer una calculadora" or "let's build a calculator"), automatically create it:
   ```bash
...
- If the user says things like *"continue what I did with Claude"*, *"continue the Kimi work"*, or *"remember when we..."*, consult the resolved `.globalcontext.md`.
- After completing a significant task or at session end, append **one concise project-state entry** to the resolved `.globalcontext.md` using the prefix `Gemini:` and the current UTC date/time.
...
If the user wants to name an existing project, run:
```bash
